...
| 73 | KMS signer Goth-path test mocking — clear critical-tier ≥95% gate on both KMS signers [D:3/B:2/U:3 → Eff:0.83] ⚠️ | ✅ | Done 2026-05-03. Added dev/test `:meck` and credential-path tests for both Ethereum and Solana CloudKMS signers. The new tests patch `Goth.fetch!/1`, drive the non-binary `cred` branch through both `get_address/6` and `sign/7`, assert the same address/signature outputs as the token-path Tesla mocks, and verify the stub is called once per test. The KMS test modules run `async: false` because `:meck` patches the `Goth` module globally. Coverage now reports both CloudKMS modules at 100% with no uncovered lines. See [CHANGELOG.md](CHANGELOG.md#unreleased). |
