...
The doctor-driven typespec sweep (2026-04-26) surfaced **three real correctness bugs** that the test suite was not exercising — tracked as Phase 0.4. Tasks 51 + 52 (Trace / TraceCall list-vs-singular spec corrections) landed 2026-04-26 — `invalid_contract` count now 11 → 9, full suite green. A subsequent Codex consultation (2026-04-26) added a fourth pre-release blocker — `Cartouche.Solana.Transaction.deserialize/1` raises `FunctionClauseError` / `MatchError` on malformed bytes instead of honoring its `{:ok, _} | {:error, _}` contract — tracked as Task 56. **Tasks 53 + 56** are the remaining pre-publish blockers on Task 6. What's left:

- **Phase 0.4 — Task 53** — V1 r/s/v unification. Critical-tier coverage prerequisite (V1 at 93.33%, must reach ≥95%). Blocks Task 6.
- **Phase 0.4 — Task 56** — Solana `deserialize/1` crash hardening. Public-API contract violation. Blocks Task 6.
...
| 53 | V1.t() Schrödinger r/s/v + latent `decode → recover_signer` crash [D:4/B:7/U:6 → Eff:1.63] 🚀 | ⬜ | **Critical-tier coverage prerequisite.** Two related defects in `lib/cartouche/transaction.ex`. (1) `V1.t()` lines 13–23 declares `r: integer(), s: integer(), v: integer()`, but `add_signature/2` (`transaction.ex:170`) stores `r`/`s` as 32-byte binaries (matched out of `<<r::binary-size(32), s::binary-size(32), v::binary>>`). `V1.new/7` lines 77–78 stores them as integer 0. `V1.decode/1` lines 141–142 stores them as integers via `:binary.decode_unsigned/1`. Three runtime shapes for the same field; only some match the spec. (2) `V1.recover_signer/2` line 218 calls `get_signature/1` line 196, which on the second clause constructs `<<r::binary-size(32), s::binary-size(32), v_enc::binary>>` — **demands binary `r`/`s`**. On a `V1.decode/1` of a signed RLP transaction, r/s are integers; this raises `ArgumentError`. Latent because `test/support/client.ex:736` decodes V1 but never routes through `recover_signer`. **Fix shape (Option B — integers throughout, single source of truth):** edit `add_signature/2` to convert incoming binary `r`/`s` via `:binary.decode_unsigned/1`; edit `get_signature/1` second clause to build the signature binary as `<<r::big-256, s::big-256, v_enc::binary>>` (integer big-endian 256-bit ≡ original 32-byte binary). V1.t() spec stays `integer()` and now matches reality. Update doctest at `transaction.ex:155–167` (currently shows `r: <<1::256>>, s: <<2::256>>` post-`add_signature` — now would show integers) and at `transaction.ex:206–211` (`recover_signer` doctest still passes because V1.encode tolerates integer or binary in RLP). **Alternative fix (Option A — binaries throughout):** flip `decode/1` to store binaries via padding/`binary_part` and update V1.t() to `r: <<_::256>>, s: <<_::256>>` — mirrors V2's storage choice. Option B is fewer edits and matches the spec already in source; recommend B unless the implementer finds a reason to prefer A. **Coverage gate:** `Cartouche.Transaction.V1` is at **93.33%** (`mix test.json --cover` 2026-04-26) — below the ≥95% critical (signing/RLP) gate. Raise coverage *first* by adding tests for the `decode → recover_signer` round-trip (the broken path), the empty-sig RLP path (`r:0, s:0`), and the `add_signature → encode → decode → recover_signer` round-trip. Then mutate. Acceptance: round-trip test passes; dialyzer drops `transaction.ex:169 invalid_contract`; existing doctests still pass with their updated values |
| 56 | Harden `Cartouche.Solana.Transaction.deserialize/1` crash paths [D:2/B:6/U:6 → Eff:3.0] 🎯 | ⬜ | `lib/cartouche/solana/transaction.ex:287–296` is specced `{:ok, t()} \| {:error, term()}` but raises on three real shapes: empty binary `<<>>` (`decode_compact_u16` → `FunctionClauseError` because both `decode_compact_u16_acc/3` clauses at lines 137 / 141 require `<<byte, rest::binary>>`), truncated compact-u16 inside any size-prefixed section (same root cause), and truncated instruction data (`read_instructions/3` lines 345–360 does five raw `<<...>> = rest` matches in the function body — `MatchError` on truncation). `read_signatures/3` and `read_pubkeys/3` already return `{:error, :insufficient_*}` via fallback `defp f(_, _, _)` clauses, so those paths are clean — fix scope is `decode_compact_u16` (add a terminator-required `defp` clause that returns `{:error, :truncated_compact_u16}` instead of falling off the end) and `read_instructions/3` (add `defp read_instructions(_, _, _), do: {:error, :insufficient_instruction_data}` fallback + wrap each in-body `<<...>> = rest` match with a `case` returning `{:error, _}` on miss). **Coverage gate:** `Cartouche.Solana.Transaction` — verify ≥80% before mutating; add `deserialize/1` tests for `<<>>`, truncation at each section boundary (sigs, pubkeys, blockhash, instructions header, instructions body), and one happy-path round-trip. Acceptance: `deserialize(<<>>) == {:error, _}`; truncated-input cases all return `{:error, _}` shape; `mix test.json --quiet` green; dialyzer clean. Discovered during Codex consultation 2026-04-26 |
...
| 57 | Fix `Cartouche.Solana.Transaction.sign_partial/2` zero-signer boundary [D:1/B:3/U:3 → Eff:3.0] 🎯 | ⬜ | `lib/cartouche/solana/transaction.ex:410–423`. When `message.header.num_required_signatures == 0`, `Enum.map(0..(num_signers - 1), ...)` evaluates `Enum.map(0..-1, ...)`. On Elixir 1.17 (project pin per `mix.exs`) `0..-1` infers step `-1` with a deprecation warning and yields `[0, -1]` → two `<<0::512>>` placeholder signatures for a 0-signer message; on Elixir 1.20+ this fully errors. Wrong shape either way. Fix: guard `num_signers == 0 → []` (clean) or rewrite as `Enum.map(0..(num_signers - 1)//1, ...)` (relies on empty-range semantics). Add boundary test asserting `sign_partial/2` on a 0-signer message returns `signatures: []`. Trigger probability is near-zero in practice (real Solana txs always have ≥1 signer / fee payer), but the fix is a one-liner with a crisp regression test. Discovered during Codex consultation 2026-04-26 |
