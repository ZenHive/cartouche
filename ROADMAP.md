...
After `0.1.0`: remaining Phase 1 spec fixes (1.2 `Wei.to_wei/1` narrowing — last item in Task 7+8+9), then parallel work through Phases 2–9 as priority dictates.
...
| 60 | `Cartouche.RPC.get_block_by_number/2` integer path crashes on real nodes [D:1/B:3/U:4 → Eff:3.5] 🎯 | ⬜ | Spec at `lib/cartouche/rpc.ex:473` declares `non_neg_integer() \| String.t()`; `get_block_by_number(55)` works in the doctest at `:450` but returns `{:error, %{code: -32602, message: "Invalid params"}}` against any real Ethereum node (verified 2026-04-28 against a live mainnet reth tunnel — `get_block_by_number(24_975_978, ethereum_node: ...)` rejects, `get_block_by_number("0x17d1a6a", ...)` and `get_block_by_number("latest", ...)` both succeed). Root cause: `send_rpc/3` Jason-encodes params verbatim (`rpc.ex:158`), so the integer is sent as `[55, false]` in JSON; the JSON-RPC spec requires `eth_getBlockByNumber`'s first param to be a quantity hex string (`"0x37"`) or a block tag (`"latest"` / `"earliest"` / `"pending"`). The mock test client tolerates the raw integer; production nodes don't. Discovered while validating the README's RPC examples against the live tunnel — the README originally chained `eth_block_number()` (returns integer) into `get_block_by_number(int)`, which would crash for any reader. README changed to use `"latest"` as a stop-gap. **Fix shape:** add `get_block_by_number(n, opts) when is_integer(n)` head that hex-encodes via `Cartouche.Hex.encode_short_hex/1` before delegating. Audit other `eth_*` callsites that forward block numbers (`get_balance/2`, `get_nonce/2`, `get_code/2`, `eth_call`'s default-block param) for the same shape — at least `get_balance/2`'s `:block_number` opt and the receipt / log block-tag handling deserve the same is_integer guard if they exist. Update the doctest at `:450` to use `"0x37"` (or add a second integer-input doctest once the hex-encoding head lands) so the test suite actually exercises a shape a real node will accept. **Coverage gate:** `Cartouche.RPC` is critical-tier — verify ≥95% before mutating; the existing test client masks this entire class of bug, so the test suite probably needs a node-strict fixture or contract-test pair. Acceptance: `Cartouche.RPC.get_block_by_number(int)` works against any real node; existing string + tag paths unchanged; companion `eth_*` block-tag callsites audited (fix in same task or split per finding). Discovered 2026-04-28 during README pre-publish validation |
