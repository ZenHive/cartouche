...
| 60 | ~~`Cartouche.RPC.get_block_by_number/2` integer path crashes on real nodes~~ [D:1/B:3/U:4 → Eff:3.5] 🎯 | ✅ | Done 2026-04-28. Public `Cartouche.Hex.encode_quantity/1` + private `Cartouche.RPC.normalize_block_param/1` applied at all 9 block-tag-forwarding callsites (`get_block_by_number/2`, `get_nonce/2`, `call_trx/2`, `estimate_gas/2`, `get_code/2`, `get_balance/2`, `get_transaction_count/2`, `trace_call/3`, both `parity_trace_call/3` clauses). Wire-format integration test via `CaptureClient` test double in `test/rpc_test.exs`. README chain restored to `eth_block_number → get_block_by_number(int)` alongside the `"latest"` form. See [CHANGELOG.md](CHANGELOG.md#unreleased) |
