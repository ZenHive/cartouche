...
| 74 | ~~`Cartouche.Wei.to_wei/1` — add `:eth` denomination with `Decimal` support~~ [D:3/B:3/U:3 → Eff:1.0] 📋 | ✅ | Done 2026-05-04. `Cartouche.Wei.to_wei/1` accepts whole-integer `:eth` and exact `Decimal` `:eth` inputs, rejects floats and negative values, raises on sub-wei precision instead of rounding, documents `:eth` as the only ETH-denomination atom, and promotes `decimal` to a direct runtime dependency. Coverage on `Cartouche.Wei` stays at 100%; see [CHANGELOG.md](CHANGELOG.md#unreleased). Closes INE-13 |
