...
**Typed cleanup is unblocked.** Task 45 pinned the current `Cartouche.Typed` helper return-shape evidence with focused ExUnit coverage: `encode_value_map/3` reaches bitstring-producing primitive and nested custom-type branches, `serialize_value_map/3` pins the sibling map-producing JSON branch, and `find_type/2` now covers the tuple match plus both raise paths. Tasks 19+20 can now rewrite the private-helper specs against observed behaviour without changing production code in the coverage PR.
...
**Phase 12 (descripex adoption) Tasks 83 + 84 landed 2026-05-06.** Task 83 (INE-28 / PR #39) annotated `Cartouche.Signer` + `Cartouche.Keys`; Task 84 (INE-31 / PR #44) annotated `Cartouche.RPC` + the 6 response/trace decoders it returns (`Block`, `Receipt`, `FeeHistory`, `DebugTrace`, `Trace`, `TraceCall`) plus their 4 nested struct modules — all 13 module entries now register in `Cartouche.__descripex_modules__/0`. Bootstrap (Task 82) landed 2026-05-05: `:descripex` promoted to direct dep, `use Descripex.Discoverable` wired into `lib/cartouche.ex`, and a validation test flunks-with-the-function-name when annotation drift hits any registered module. Tasks 85-88 remain — four `[P]`-eligible annotation passes that can run in parallel sessions; Task 89 wires the manifest export once they all land. Highest-leverage remaining: Task 85 (Transaction + V1/V2/V3/V4 nested), Task 87 (Solana stack — 9 modules), Task 88 (Hex/Erc20/Sleuth + primitives — 9 modules).
...
| 91 | `Cartouche.Solana.Transaction.sign/2` — reject signer-count mismatch against `message.header.num_required_signatures` `[CX]` [D:1/B:3/U:3 → Eff:3.0] 🎯 | ⬜ | **Surfaced during INE-34 / PR #45 review by CodeRabbit (Major).** `Cartouche.Solana.Transaction.sign(message, seeds)` (`lib/cartouche/solana/transaction.ex` ~line 521) signs once per supplied seed and serializes that count, so callers can produce a "full transaction" whose signature array length doesn't match `message.header.num_required_signatures` — Solana will reject it on submission, but cartouche emits it without flagging. Out of scope on PR #45 per the issue body's "Modifying any function bodies" exclusion (annotation-only PR). Critical-tier (≥95% coverage gate) — `Cartouche.Solana.Transaction` currently 98.97% post-Task-56. Acceptance: validate `length(seeds) == message.header.num_required_signatures` at the top of `sign/2` and raise `ArgumentError` (or return `{:error, :signer_count_mismatch}` — pick one matching the existing error contract on the module) on mismatch; add a regression test asserting the error path against a synthetic `Message{header: %{num_required_signatures: 2}}` with a 1-element seed list. Pre-mutation gate: confirm `Cartouche.Solana.Transaction` ≥ 95% before mutating |
| 92 | `Cartouche.Solana.Transaction.add_signature/3` — guard `index` against `length(transaction.signatures)` before `List.replace_at/3` `[CX]` [D:1/B:3/U:3 → Eff:3.0] 🎯 | ⬜ | **Surfaced during INE-34 / PR #45 review by CodeRabbit (Major).** `Cartouche.Solana.Transaction.add_signature(transaction, index, signature)` (`lib/cartouche/solana/transaction.ex` ~line 615) calls `List.replace_at/3` on `transaction.signatures` without bounds-checking `index`. `List.replace_at/3` silently returns the unchanged list on out-of-bounds (verified against Elixir 1.18 `List` source — the documented safe behavior), so `add_signature` reports success while leaving the signature slot unfilled. In sponsored-transaction flows where `add_signature` fills missing signatures one-by-one, this can leave a partially-signed transaction without any indication to the caller. Out of scope on PR #45 per the issue body's "Modifying any function bodies" exclusion. Critical-tier (≥95% coverage gate). Acceptance: add a guard requiring `is_integer(index) and index >= 0 and index < length(transaction.signatures)` and raise `ArgumentError` (or return `{:error, :invalid_signature_slot}` — match the module's existing error contract) on invalid index; regression test asserts the error path against an `index = length(transaction.signatures) + 1` callsite. Pre-mutation gate: confirm `Cartouche.Solana.Transaction` ≥ 95% before mutating. **Bundle candidate with Task 91** — same module, same critical-tier gate, same fix shape (precondition + raise) |
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 19+20 | Phase 4 Typed internal-function specs — rewrite + visibility judgment `[CX]` [D:3/B:2/U:3 → Eff:0.83] ⚠️ | ⬜ | **Unblocked by Task 45.** Read current impls, derive the true return types from the focused evidence now in `test/typed_test.exs`, rewrite both `@spec` lines (Task 19). If these really are internal, add `@doc false` — keeps the `@spec` for dialyzer but removes from generated docs. Then decide whether either fn is meant to be public API (Task 20); if so, adjust impl to match the documented intent instead of changing the spec. Judgment call; easier under fork ownership since we decide |
