...
Bugs surfaced during the Task 40 staged review (2026-04-26) that don't block `0.1.0` and warrant their own commits.
...
| 49 | Resolve `Cartouche.Transaction.V2.encode/1` spec duplication — widen `t()` vs introduce `unsigned_t()` [D:2/B:2/U:3 → Eff:1.25] 📋 | ⬜ | The doctor-driven typespec sweep (2026-04-26) speced `V2.encode/1` as `t() \| %__MODULE__{... signature_y_parity: nil, signature_r: nil, signature_s: nil}` — the second arm of the union duplicates the entire struct shape inline because `V2.t()` (line 232) declares `signature_y_parity: boolean()` (non-nil-admitting), which excludes the legitimate unsigned form that `encode/1` and the four `build_trx_v2` paths produce. Two cleaner options: (a) widen `V2.t()` so `signature_y_parity: boolean() \| nil`, `signature_r: <<_::256>> \| nil`, `signature_s: <<_::256>> \| nil` — same shape callers see today, just admitted by the type — or (b) introduce a sibling `@type unsigned_t :: %__MODULE__{... signature_*: nil}` and use a union of `t() \| unsigned_t()` everywhere `encode` / `build_*` flow. (a) is simpler and matches dialyzer's inferred shape; (b) preserves the "fully signed" invariant on `t()`. Audit V1 too — same shape exists for `v: 0, r: 0, s: 0` in `Cartouche.Transaction.V1`. Discovered during the staged review of the typespec sweep |
