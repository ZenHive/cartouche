...
**Phase 1.4 closeout landed 2026-04-28** — the four `Cartouche.Hex` `@spec` corrections (already shipped under `8d4bc18`) are now grounded with focused ExUnit assertions and the missing failure-path doctests; bundled coverage push lifts `Cartouche.Hex` 94.29% → 100%. Dialyzer remains clean on `hex.ex`. Onchain `@dialyzer {:no_match}` strip becomes load-bearing once `0.1.x` ships.

**Phase 1 closed 2026-04-29** — Phase 1.2 `Wei.to_wei/1` narrowing (last Task 7+8+9 item) landed bundled with Phase 0.5 Task 55 (`Trace.deserialize/1` missing/`nil` `traceAddress` hardening). Suite 778/778 green; dialyzer clean on `wei.ex` and `trace.ex`; total `invalid_contract` count holds at 8 (the remaining 6 are tracked under Task 54 — Sleuth — and 2 under Tasks 19+20 — Typed). All four Phase 1 spec corrections (1.1 RecoveryBit, 1.2 Wei, 1.3 Signer, 1.4 Hex) are now in `[Unreleased]` — the onchain `@dialyzer {:no_match}` strip across `Onchain.Hex` / ABI / ERC / ENS / Multicall callers becomes load-bearing the moment cartouche `0.1.x` ships.

**Mainnet integration suite landed 2026-04-30 (Task 61).** `test/rpc_integration_test.exs` against the local archive node tunnel; opt-in via `mix integration`; per-call client/URL injection (CCXT-style local-object pattern) — no app-env mutation, tests stay `async: true`. Caught **two real Task-60-class wire-format bugs the mock had been masking** at first run: `get_block_by_hash/2` was missing the required `fullTransactionObjects` second wire param, and `to_call_params/2` for V1 encoded empty `data` as `"0x0"` (invalid for the DATA type). Both fixed in `[Unreleased]`. Decoder gaps surfaced filed as Tasks 62–68 — trace methods (62), Block fork-era fields (63 base_fee, 64 withdrawals, 65 Cancun), Block tx-details (66), Receipt blob fields (67), and DebugTrace EIP-7702 opcodes (68).

After `0.1.x`: parallel work through Phases 2–9 as priority dictates. Highest-Eff unblocked candidates are Task 63 (Block.base_fee_per_gas, Eff:4.0), Task 47 (exclude IConsole from coverage measurement, Eff:2.0), Task 64 (Block.withdrawals, Eff:2.0), Task 65 (Block Cancun fields, Eff:2.0), Tasks 14+15+35 (RPC error-shape widening + `Jason.encode!` rescue, Eff:1.75), and Task 27+28 (Finch 0.19→0.21, Eff:1.75).
...
`7+8+9` Phase 1.1–1.3 ✅ · `10+11+12+13` Phase 1.4 Hex ✅ · `14+15+35` RPC error shapes · `16+17+18` Trace specs · `19+20` Typed specs · `21+22` VM cascade · `24+25` KMS · `27+28` Finch · `29+30` V2 dedup.
...
| 62 | v2 traces — integration anchors for `trace_transaction`, `trace_call`, `trace_callMany`, `debug_traceCall` [D:5/B:5/U:5 → Eff:1.0] 📋 | ⬜ | Extend `test/rpc_integration_test.exs` with the four trace methods deferred from Task 61's v1 scope. Anchor selection caveat: node-implementation variance — geth/reth/erigon differ on internal trace fields, so picking mainnet anchor txs that produce stable shape across implementations is non-trivial. Avoid anchors involving post-EIP-7702 traces until Task 68 lands (`Cartouche.DebugTrace.StructLog` opcode whitelist will currently raise `ArgumentError` on `AUTH`/`AUTHCALL`). Pre-implementation: probe the live archive node for a known-stable historical tx and confirm trace shape against the existing `Cartouche.Trace.deserialize/1` and `Cartouche.DebugTrace.StructLog.deserialize/1` decoders. Coverage gate: `Cartouche.Trace` is at 100%, `Cartouche.TraceCall` likely OK; `Cartouche.DebugTrace.StructLog` needs check |
| 63 | `Cartouche.Block` — add `base_fee_per_gas` (London+) [D:1/B:3/U:5 → Eff:4.0] 🎯 | ⬜ | Decoder gap surfaced by Task 61. `lib/cartouche/block.ex:10–30` `defstruct` has no `:base_fee_per_gas` field; the integration suite pins this gap with `refute Map.has_key?(b, :base_fee_per_gas)` at `test/rpc_integration_test.exs` (post-London / post-Shanghai / post-Cancun blocks). Add the field, decode `params["baseFeePerGas"]` (hex-quantity) in `deserialize/1`, update `@type t` and the doctest. Strengthen the integration test's TODO assertion (`assert b.base_fee_per_gas > 0`). Pre-mutation gate: `Cartouche.Block` coverage check |
| 64 | `Cartouche.Block` — add `withdrawals_root` and `withdrawals` (Shanghai+) [D:2/B:3/U:5 → Eff:2.0] 🚀 | ⬜ | Decoder gap surfaced by Task 61. Shanghai upgrade (block ≥ 17,034,870) added validator withdrawals — `withdrawals` is a list of `%{index, validator_index, address, amount}` and `withdrawals_root` is the 32-byte trie root. Add both fields to `Cartouche.Block.defstruct` + `@type t`; either embed a `Cartouche.Block.Withdrawal` substruct or use a plain map. Decode in `deserialize/1`. Strengthen the integration test's TODO assertions at `test/rpc_integration_test.exs` (post-Shanghai block). Pre-mutation gate: coverage check |
| 65 | `Cartouche.Block` — add Cancun fields (`parent_beacon_block_root`, `blob_gas_used`, `excess_blob_gas`, `mix_hash`) [D:2/B:3/U:5 → Eff:2.0] 🚀 | ⬜ | Decoder gap surfaced by Task 61. Cancun upgrade (block ≥ 19,426,587, EIP-4844 + EIP-4788) added beacon-chain root + blob-gas accounting. Add the four fields to `Cartouche.Block.defstruct` + `@type t`. Note `mix_hash` predates Cancun but is also missing — it has been on the wire since pre-Merge; surfaced now as part of the same gap audit. Strengthen the integration test's TODO assertions at `test/rpc_integration_test.exs` (post-Cancun block) |
| 66 | `Cartouche.Block.transactions` — implement `include_transaction_details: true` [D:5/B:4/U:5 → Eff:0.9] ⚠️ | ⬜ | `Cartouche.RPC.get_block_by_number/2` and `get_block_by_hash/2` accept the `:include_transaction_details` opt and forward `true`/`false` to the node, but `Cartouche.Block.deserialize/1` currently hardcodes `transactions: []` regardless of what the node returned. When `true`, the node returns full transaction objects (V1 or V2 shape depending on tx type) instead of just hashes. Implement: dispatch on the shape of each `params["transactions"]` element (string → tx hash, map → full V1/V2 object). Reuse existing `Cartouche.Transaction.V1.decode/1` / `V2.decode/1` if shapes line up, or write a JSON-shape decoder. Add an integration anchor for the `true` shape. Pre-mutation gate: `Cartouche.Block` coverage check; this is a contract-narrowing on existing public API. **U bumped 4→5 (2026-04-30):** `:include_transaction_details` is now an explicitly-documented public option on both `get_block_by_*` `@doc`s — every user who tries `true` hits `transactions: []` and has to read the caveat |
| 67 | `Cartouche.Receipt` — add EIP-4844 blob fields (`blob_gas_used`, `blob_gas_price`) [D:2/B:3/U:4 → Eff:1.75] 🚀 | ⬜ | Decoder gap surfaced by Task 61. Cancun receipts for blob-carrying transactions (type-3) include `blob_gas_used` and `blob_gas_price` (alongside `effective_gas_price`). Add to `Cartouche.Receipt.defstruct` + `@type t`; decode in `deserialize/1`. Add a type-3 (blob-tx) anchor to the integration suite — pick a known mainnet blob tx after EIP-4844 activation (block ≥ 19,426,587). Strengthen the type-2 receipt test's TODO assertions; add a new "type-3 (blob) receipt" test |
| 68 | `Cartouche.DebugTrace.StructLog` — add EIP-7702 opcodes to the closed whitelist (`AUTH`, `AUTHCALL`) [D:1/B:2/U:3 → Eff:2.5] 🎯 | ⬜ | The closed whitelist in `lib/cartouche/debug_trace.ex` was hardened in `[Unreleased]` (Cancun-era opcodes only) to prevent atom-table DOS. EIP-7702 (Pectra, expected mainnet activation 2025) adds `AUTH` (0xf6) and `AUTHCALL` (0xf7). Without these in the whitelist, any `eth_debug_traceCall` against a post-Pectra tx that uses 7702 raises `ArgumentError`. Add both opcodes to the compile-time map. Verify against go-ethereum's `core/vm/opcodes.go` master once Pectra lands (current target block TBD). This task is pre-emptive — file now so the integration suite can pick a 7702 anchor as soon as activation happens |
