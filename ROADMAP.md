...
| 77 | `Cartouche.Solana.RPC` coverage push — pre-existing untested option/filter/encoding paths `[CSR]` [D:2/B:3/U:4 → Eff:1.75] 🚀 | ⬜ | Discovered during INE-25 / PR #37 review (2026-05-05). Cursor's first impl commit (`c8ace87`) added scope-creep tests that covered these paths, then self-removed in `209ab96` ("Remove out-of-scope Solana RPC tests") since they fall outside INE-25's "error-shape widening" scope. The lib code IS pre-existing on `development` (it lives outside the PR's `+32 -7` lib diff) and is untested there — coverage gap is pre-existing, not introduced by INE-25. **Untested paths in `lib/cartouche/solana/rpc.ex`** (line numbers from `fab306b`): (a) `encoding_string/1` clauses at `:127–131` covering `:base58`, `:base64`, `:"base64+zstd"`, `:json_parsed`, and binary-string passthrough — exercised by `:encoding` opt on `get_account_info/2` (`:120`), `get_multiple_accounts/2` (analogous), and `get_transaction/2` (`:261`); (b) `get_token_accounts_by_owner/3` `ArgumentError` raise at `:382` when neither `:mint` nor `:program_id` filter is provided; (c) `send_transaction/2` opt-handling around `:encoding`, `:skip_preflight`, `:preflight_commitment`, `:max_retries`. **Reference for tests** — the deleted tests live at `git show c8ace87:test/solana/rpc_test.exs` (search for `"accepts supported account encodings"`, `"accepts explicit transaction encodings"`, `"requires a mint or program id filter"`, `"accepts base58 encoding and send options"`). Adapt them; don't simply re-introduce verbatim — verify each assertion still pins the current dev-branch behavior. **Scope:** test additions only, no `lib/cartouche/solana/rpc.ex` mutation. Pre-implementation: `Cartouche.Solana.RPC` coverage check (test-only addition is exempt from the ≥80% mutation gate). Acceptance: each listed path has at least one ExUnit assertion that fails if the path regresses; `mix test.json --quiet` green; `mix credo --strict --format json` clean on touched test file |
