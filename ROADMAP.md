...
The doctor-driven typespec sweep (2026-04-26) surfaced **three real correctness bugs** that the test suite was not exercising — tracked as Phase 0.4. Tasks 51 + 52 (Trace / TraceCall list-vs-singular spec corrections) landed 2026-04-26 — `invalid_contract` count now 11 → 9, full suite green. **Task 53** (V1 r/s/v unification + latent `recover_signer` ArgumentError) is the only remaining pre-publish blocker on Task 6. What's left:
...
Three real bugs surfaced during the doctor-driven typespec sweep (2026-04-26) that the type system caught but the test suite was not exercising. Two are consumer-facing MatchError potential (Trace / TraceCall list-vs-singular type mismatch); one is a latent crash on a public API path (`V1.decode → recover_signer → ArgumentError` on signed RLP). All three must land before Task 6 (`mix hex.publish`).
...
## Phase 0.5: Generator hardening (post-0.1.0 follow-ups)

Bugs surfaced during the Task 40 staged review and subsequent Phase 0.4 work (2026-04-26) that don't block `0.1.0` and warrant their own commits.
