...
**Phase 12 (descripex adoption) Task 83 landed 2026-05-06 via INE-28 / PR #39.** `Cartouche.Signer` + `Cartouche.Keys` annotated with `api()` metadata; both modules now in `Cartouche.__descripex_modules__/0`. Bootstrap (Task 82) landed 2026-05-05: `:descripex` promoted to direct dep, `use Descripex.Discoverable` wired into `lib/cartouche.ex`, and a validation test flunks-with-the-function-name when annotation drift hits any registered module. Tasks 84-88 remain — five `[P]`-eligible annotation passes that can run in parallel sessions; Task 89 wires the manifest export once they all land. Largest single-task surface: Task 84 (RPC + 6 response decoders, 7 modules total).
