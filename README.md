...
**Upcoming — Phase 12 (descripex adoption).** The top-level `Cartouche` module now exposes `describe/0,1,2` for machine-readable API discovery via [`descripex`](https://hexdocs.pm/descripex). The registered module list is empty until per-module annotation passes (Tasks 83-88) land — see ROADMAP.md.
