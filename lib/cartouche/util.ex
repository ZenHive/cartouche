defmodule Cartouche.Util do
  ...
  ## Examples
    ...
    iex> Cartouche.Util.decode_hex("0x1122")
    {:ok, <<0x11, 0x22>>}

    iex> Cartouche.Util.decode_hex("0x1")
    {:ok, <<0x1>>}

    iex> Cartouche.Util.decode_hex("0xGG")
    :error
  """
  @spec decode_hex(String.t()) :: {:ok, binary()} | :error
  @deprecated "Use Cartouche.Hex.decode_hex/1 instead"
  ...
  ## Examples
    ...
    iex> Cartouche.Util.decode_sized_hex!("0x1122", 2)
    <<0x11, 0x22>>

    iex> Cartouche.Util.decode_sized_hex!("0x1122", 3)
    ** (RuntimeError) mismatch byte size. expected 3, got: 2

    iex> Cartouche.Util.decode_sized_hex!("0xGG", 3)
    ** (RuntimeError) invalid hex
  """
  @spec decode_sized_hex!(String.t(), integer()) :: binary() | no_return()
  @deprecated "Use Cartouche.Hex.decode_sized!/2 instead"
  ...
  ## Examples
    ...
    iex> Cartouche.Util.decode_word!("0x00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff")
  ...
  ## Examples
    ...
    iex> Cartouche.Util.decode_hex_number!("0x11223344")
    0x11223344
  """
  ...
  ## Examples
    ...
    iex> Cartouche.Util.encode_hex(<<0x11, 0x22>>)
    "0x1122"

    iex> Cartouche.Util.encode_hex(<<0xc>>)
    "0x0C"

    iex> Cartouche.Util.encode_hex(<<0xc>>, true)
    "0xC"

    iex> Cartouche.Util.encode_hex(<<0x0>>, true)
    "0x0"
  """
  @deprecated "Use Cartouche.Hex.encode_short_hex/1 instead"
  ...
  ## Examples
    ...
    iex> Cartouche.Util.encode_bytes(257, 4)
    <<0, 0, 1, 1>>

    iex> Cartouche.Util.encode_bytes(nil, 4)
    nil
  """
  ...
  ## Examples
    ...
    iex> Cartouche.Util.pad(<<1, 2>>, 2)
    <<1, 2>>

    iex> Cartouche.Util.pad(<<1, 2>>, 4)
    <<0, 0, 1, 2>>

    iex> Cartouche.Util.pad(<<1, 2>>, 1)
    ** (FunctionClauseError) no function clause matching in Cartouche.Util.pad/2
  """
  ...
  ## Examples
    ...
    iex> Cartouche.Util.get_eth_address(public_key)
  ...
  ## Examples
      ...
      iex> Cartouche.Util.to_wei(100)
      100

      iex> Cartouche.Util.to_wei({100, :gwei})
      100000000000
  """
  ...
  ## Examples
      ...
      iex> Cartouche.Util.parse_chain_id(5)
      5

      iex> Cartouche.Util.parse_chain_id(:unichain)
      130
  """
  ...
  ## Examples
      ...
      iex> Cartouche.Util.checksum_address("0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaed")
      "0x5aAeb6053F3E94C9b9A09f33669435E7Ef1BeAed"

      iex> Cartouche.Util.checksum_address("0xFB6916095CA1DF60BB79CE92CE3EA74C37C5D359")
      "0xfB6916095ca1df60bB79Ce92cE3Ea74c37c5d359"

      iex> Cartouche.Util.checksum_address("0xdbf03b407c01e7cd3cbea99509d93f8dddc8c6fb")
  ...
  ## Examples
      ...
      iex> Cartouche.Util.nibbles(<<0xF5,0xE6,0xD0>>)
  ...
  defmodule RecoveryBit do
    ...
    * `:eip155`: In the range `{35+chain_id*2,35+chain_id*2+1}`, as defined in EIP-155
    ...
    ## Examples
        ...
        iex> Cartouche.Util.RecoveryBit.normalize(28, :eip155)
        46

        iex> Cartouche.Util.RecoveryBit.normalize(1, :ethereum)
        28

        iex> Cartouche.Util.RecoveryBit.normalize(45, :base)
        0
    """
    ...
    ## Examples
        ...
        iex> Cartouche.Util.RecoveryBit.normalize_signature(<<1::256, 2::256, 28>>, :eip155)
        <<1::256, 2::256, 46>>

        iex> Cartouche.Util.RecoveryBit.normalize_signature(<<1::256, 2::256, 1>>, :ethereum)
        <<1::256, 2::256, 28>>

        iex> Cartouche.Util.RecoveryBit.normalize_signature(<<1::256, 2::256, 45>>, :base)
        <<1::256, 2::256, 0>>
    """
    ...
    ## Examples
        ...
        iex> Cartouche.Util.RecoveryBit.recover_base(0)
        0

        iex> Cartouche.Util.RecoveryBit.recover_base(1)
        1

        iex> Cartouche.Util.RecoveryBit.recover_base(27)
        0

        iex> Cartouche.Util.RecoveryBit.recover_base(28)
        1

        iex> Cartouche.Util.RecoveryBit.recover_base(45)
        0

        iex> Cartouche.Util.RecoveryBit.recover_base(46)
        1

        iex> Cartouche.Util.RecoveryBit.recover_base(47)
        ** (RuntimeError) Invalid EIP-155 Signature: recovery_bit=47, chain_id=5

        iex> Cartouche.Util.RecoveryBit.recover_base(2)
        ** (FunctionClauseError) no function clause matching in Cartouche.Util.RecoveryBit.recover_base/1
    """
